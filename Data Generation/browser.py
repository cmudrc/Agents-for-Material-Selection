...
class SimpleTextBrowser:
    ...
    def __init__(
        ...
        google_cse_id: Optional[Union[str, None]] = None,
        google_api_key: Optional[Union[str, None]] = None,
    ...
    def set_address(self, uri_or_path: str, filter_year: Optional[int] = None) -> None:
        ...
        if uri_or_path == "about:blank":
            ...
            self._google_cse_search(uri_or_path[len("google:"):].strip(), filter_year=filter_year)
    ...
    def _google_cse_search(self, query: str, filter_year: Optional[int] = None) -> None:
        if not hasattr(self, "google_cse_id") or not hasattr(self, "google_api_key"):
            raise ValueError("Missing Google Custom Search Engine ID or API key.")
        ...
        for idx, page in enumerate(results["items"], 1):
            date_published = page.get("pagemap", {}).get("metatags", [{}])[0].get("datePublished", "")
            source = page.get("displayLink", "")
            snippet = page.get("snippet", "")

            entry = f"{idx}. [{page['title']}]({page['link']})\n"
            if date_published:
                entry += f"Date published: {date_published}\n"
            if source:
